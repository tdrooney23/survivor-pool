function doGet(e) {
  try {
    ...
    return ContentService
      .createTextOutput(JSON.stringify({ participants, picks, r32picks, s16picks, paid }))
      .setMimeType(ContentService.MimeType.JSON);
  } catch(err) {
    return ContentService
      .createTextOutput(JSON.stringify({ error: err.toString(), participants: [], picks: [], r32picks: [], s16picks: [], paid: [] }))
      .setMimeType(ContentService.MimeType.JSON);
  }
}
